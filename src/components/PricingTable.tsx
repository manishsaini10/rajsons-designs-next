import type { PricingPlan } from "@/lib/site-data";

interface PricingTableProps {
  plans: PricingPlan[];
  note?: string;
}

export default function PricingTable({ plans, note }: PricingTableProps) {
  return (
    <div className="my-8">
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {plans.map((plan) => (
          <div
            key={plan.name}
            className={`group relative flex flex-col rounded-2xl border-2 p-6 md:p-8 transition-all duration-300 hover:-translate-y-2 ${
              plan.highlighted
                ? "border-[#f7941e] bg-gradient-to-b from-[#fff8f0] to-white shadow-xl shadow-[#f7941e]/10 hover:shadow-2xl hover:shadow-[#f7941e]/20"
                : "border-gray-200 bg-white shadow-sm hover:shadow-xl hover:border-gray-300"
            }`}
          >
            {plan.highlighted && (
              <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-[#f7941e] to-[#e5840e] px-5 py-1.5 text-xs font-bold text-white shadow-lg">
                  <svg className="h-3 w-3" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                  POPULAR
                </span>
              </div>
            )}
            <h3 className={`mb-1 text-lg font-bold ${plan.highlighted ? "text-[#f7941e]" : "text-[#1e3a5f]"}`}>{plan.name}</h3>
            <div className="mb-6">
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-extrabold text-[#1e3a5f]">{plan.price}</span>
                <span className="text-sm text-gray-400">/{plan.priceUsd}</span>
              </div>
              <p className="text-xs text-gray-400 mt-0.5">{plan.duration}</p>
            </div>
            <ul className="mb-8 flex-1 space-y-3">
              {plan.features.map((f, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-gray-600">
                  <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${plan.highlighted ? "bg-[#f7941e]/10 text-[#f7941e]" : "bg-[#1e3a5f]/10 text-[#1e3a5f]"}`}>
                    <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                  </span>
                  {f}
                </li>
              ))}
            </ul>
            <a
              href="/contact-us.html"
              className={`mt-auto block w-full rounded-xl py-3.5 text-center text-sm font-bold transition-all ${
                plan.highlighted
                  ? "bg-gradient-to-r from-[#f7941e] to-[#e5840e] text-white shadow-md hover:shadow-xl hover:shadow-[#f7941e]/30"
                  : "bg-[#1e3a5f] text-white hover:bg-[#2a4a70] shadow-md hover:shadow-lg"
              }`}
            >
              Get Started
            </a>
          </div>
        ))}
      </div>
      {note && <p className="mt-6 text-xs text-gray-400 text-center">{note}</p>}
    </div>
  );
}
